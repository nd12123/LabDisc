...
javascriptGenerator.forBlock["display_var"] = function (block) {
  ...
  if (sensorId) {
    const [sensorName, sensorUnit] = getSensorDisplay(Number(sensorId));
...
javascriptGenerator.forBlock["bar"] = function (block) {
  ...
  if (sensorId) {
    ...
    return `displayBar("${blockId}", ${min}, ${max}, null, "${color}", ${steps}, "${sensorName}", "${sensorUnit}", "${pos}", "${sensorId}");\n`;
  } else {
    return `displayBar("${blockId}", ${min}, ${max}, ${variable}, "${color}", ${steps}, "", "", "${pos}");\n`;
  }
};
